...
const forestTypeOptions = ['Spruce', 'Pine', 'Mixed Coniferous', 'Mixed Deciduous'];
const vegetationTypeOptions = ['Tall herbs', 'Low herbs', 'No field layer', 'Broad grasses', 'Narrow grasses', 'Blueberry', 'Lingonberry', 'Crowberry/heather'];
  ...
  for (const { geo, forest, veg, age } of allCombinations) {
      try {
        ...
        if (!data || data.length === 0) {
          console.log('No data fetched for combination', { geo, forest, veg, age });
          continue;
        }

        const filename = `data-${geo}-${forest}-${age}-${veg}.json`;
        fs.writeFileSync(path.join(__dirname, `../static/${filename}`), JSON.stringify(data));
        console.log(`Data pre-fetching complete for ${filename}.`);
      } catch (error) {
        console.error('Error during data prefetch for combination', { geo, forest, veg, age }, ':', error);
      }
  }
}
