<template>
  <div
    class="p-5 w-fit bg-neutral-100 dark:bg-neutral-800 dark:bg-opacity-100 border-[1px] dark:border-stone-700 border-stone-300 rounded-xl"
  >
    <div>
      <h1 size="md" class="mb-4">Beräkna</h1>

      <!-- Form wrapping the inputs and calculate button -->
      <form @submit.prevent="calculate">
        <div>
          <h1 size="sm" class="mb-4">Hyggestorlek</h1>
          <div
            v-for="(harvest, index) in harvestAreas"
            :key="'harvest-' + index"
            class="flex gap-4"
          >
            <UInputMenu
              ...
              type="number"
              step="any"
              label="Hyggestorlek (ha)"
              v-model="harvest.area"
            />
            <UButton
              color="muted"
              v-if="harvestAreas.length > 1"
              class="mt-2 w-5 h-5"
              shape="full"
              @click="removeHarvestArea(index)"
            />
          </div>
          <UButton
            color="muted"
        ...
        <div>
          <br />
          <h1 size="sm" class="mb-4">Hänsyn</h1>
          <div
            v-for="(group, index) in treeGroups"
            :key="index"
            class="flex gap-4"
          >
            <UInputMenu
              ...
              type="number"
              step="any"
              label="Gruppställda hänsynsträd (m²)"
              v-model="group.area"
            />
            <!-- Add a condition to show the remove button only if more than one group exists -->
            <UButton
              color="muted"
              v-if="treeGroups.length > 1"
              class="mt-2 w-5 h-5"
              shape="full"
              @click="removeTreeGroup(index)"
            />
          </div>
          <UButton
            color="muted"
        ...
        <div class="flex">
          <UInputMenu
            shape="curved"
            type="number"
            label="Enstaka hänsynsträd"
            v-model="numTrees"
          />
        </div>

        <!-- Calculate button now submits the form -->
        <UButton color="primary" class="mt-2" shape="curved" type="submit"
          >Beräkna</UButton
        >
      </form>
    </div>
  ...
  <div
    v-if="result"
    class="p-5 w-fit mt-4 bg-neutral-100 dark:bg-neutral-800 dark:bg-opacity-100 border-[1px] dark:border-stone-700 border-stone-300 rounded-xl"
  >
    <div>
    ...
    calculate() {
      ...
      for (const group of this.treeGroups) {
        if (group.area > 0) {
          ...
          const groupDiameter =
            Math.sqrt(group.area / Math.PI) * 2 + this.treeHeight;
      ...
      for (const harvest of this.harvestAreas) {
        ...
        const harvestAreaDiameter =
          Math.sqrt(harvestAreaInSquareMeters / Math.PI) * 2;
        // Apply the edge effect by reducing the diameter
        const modifiedDiameter = Math.max(
          0,
          harvestAreaDiameter - this.treeHeight
        );
      ...
      const totalFungiAreaAffected = Math.max(
        0,
        totalHarvestAreaWithEdgeEffect -
          totalAreaTreeGroup -
          totalAreaIndividualTree
      );
      ...
      if (totalHarvestAreaWithoutEdgeEffect > 0) {
        const percentage =
          (1 - totalFungiAreaAffected / totalHarvestAreaWithoutEdgeEffect) *
          100;
