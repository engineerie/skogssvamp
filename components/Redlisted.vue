...
<template>
  <div
    class="relative p-6 backdrop-blur-3xl rounded-xl bg-neutral-50 dark:bg-neutral-900 dark:bg-opacity-60 border-[1px] dark:border-neutral-800 border-stone-200"
  >
    <div class="grid grid-cols-4 mb-8">
      <div class="col-span-3">
        <div>
          <h1 size="md">Naturvårdsarter</h1>
          <p size="sm" class="mb text-neutral-500">
            Rödlistade svampar och signalarter som kan förekomma i denna miljö.
          </p>
        </div>
      </div>
      <div class="col-span-1 flex justify-end">
        <UButton shape="full" @click="$emit('enlarge')" class="my-2">
          <Icon name="material-symbols:open-in-full" class="size-5" />
        </UButton>
        <div
          class="w-12 h-12 mb-4 text-red-500 flex justify-center items-center rounded-full border-red-500"
        >
          <img
            src="~/assets/icons/redlisted3.svg"
            class="h-8 w-8"
            alt="Redlisted Icon"
          />
        </div>
    ...
    <!-- List of mushrooms -->
    <div
      v-for="(mushroom, index) in paginatedMushrooms"
      :key="index"
      class="flex justify-between items-center my-4"
    >
      <div class="flex items-center">
        <div
          :class="getStatusColor(mushroom.status)"
          class="h-8 w-8 rounded-full flex items-center justify-center mr-4"
        >
          <span class="text-white font-semibold">{{
            getStatusAbbreviation(mushroom.status)
          }}</span>
        </div>
        <div>
          <p class="text-neutral-500">{{ mushroom.name }}</p>
          <p class="text-xs text-neutral-500">{{ mushroom.status }}</p>
        </div>
      </div>
      <div class="flex items-center">
        <!-- Status circle -->
        <UButton
          to="https://artfakta.se/artinformation/taxa/sarcosoma-globosum-1445/detaljer"
          target="_blank"
          shape="full"
          size="sm"
        >
        ...
        <!-- <UIcon size="xs" shape="full" class="border-muted-200 dark:border-muted-600 dark:bg-muted-700 border bg-white text-gray-500">
