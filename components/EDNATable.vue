<template>
  ...
  >
    ...
    <div
      v-if="data"
      id="scrollbar"
      class="overscroll overflow-y-scroll h-96 pl-2"
    >
      <div
      ...
      >
        ...
        <div v-if="row.matsvamp === 1">
          <Icon
            name="icon-park-twotone:pot"
            class="h-8 w-8 mr-14 text-yellow-500"
          />
        </div>
      </div>
    </div>
...
onMounted(async () => {
  const response = await fetch(
    `/api/fetchData?geography=${props.geography}&forestType=${props.forestType}&vegetationType=${props.vegetationType}&standAge=${props.standAge}`
  );
