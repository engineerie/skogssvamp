...
<template>
    ...
    <div class="flex justify-between mb-2 items-end">
        <div class="flex items-end">
          <div class="dark:opacity-90 w-12 h-12 ml-2 mr-3 rounded-lg text-violet-500 flex justify-center items-center">              
            ...
            <BaseHeading size="3xl" weight="medium" class="text-neutral-800 dark:text-neutral-300">Mykorrhizasvampar</BaseHeading>
          </div> 
    ...
    <div class="relative pt-3 backdrop-blur-3xl rounded-xl bg-white bg-opacity-80 dark:bg-neutral-900 dark:bg-opacity-60 border  dark:border-neutral-800 border-stone-200 ">
      <div class="grid grid-cols-2 mb-8">
        ...
        <div v-if="filteredData" class="col-span-6 -mt-12">
            <div class="h-fit pt-2 ">
              ...
              <UTable 
              :sort-button="{ color: 'text-neutral-700 dark:text-neutral-300', size: 'xl', }"
              :ui="{ 

                td: { padding: 'py-6', size: 'text-md' ,  color: 'text-neutral-500 dark:text-neutral-400', },
                tbody: 'divide-y divide-neutral-200 dark:divide-neutral-800', divide: 'divide-y divide-neutral-300 dark:divide-neutral-700', 
                tr: { base: '', selected: 'bg-neutral-100 dark:bg-neutral-900', active: 'hover:bg-neutral-100 dark:hover:bg-neutral-800 active:bg-neutral-200 dark:active:bg-neutral-900', }, 
              }" 
              :columns="columns" 
              :rows="paginatedData"
              @select="selectRow">
              <template #snamn-data="{ row }">
                ...
                <template #matsvamp-data="{ row }">
                  <div v-if="row.matsvamp === 1">
                    <Icon name="fluent:food-16-filled" class="h-8 w-8 text-yellow-500 -my-2" />
                  </div>
                  <div v-if="row.matsvamp === 0">
                  </div>
                </template>
                <!-- ...other columns... -->
              </UTable>
              <div class="flex justify-between items-center p-5 border-t-[1px] border-neutral-200 dark:border-neutral-800">
                <div>
                  <!-- Display the current range and total items -->
                  <BaseProse class="text-sm">Visar {{ startItem }} till {{ endItem }} av {{ totalItems }} arter</BaseProse>
                </div>
                <div>
                  ...
                  >
                    ...
                    <template #prev="{ onClick }">
                        <UButton icon="i-heroicons-chevron-left-20-solid"  size="lg" color="white" :ui="{ rounded: 'rounded-full dark:border-neutral-800' }" class="rtl:[&_span:first-child]:rotate-180 dark:bg-neutral-900 border-[0.5px]" @click="onClick" />
                    </template>
              
                    <!-- Custom next button -->
                    <template #next="{ onClick }">
                        <UButton icon="i-heroicons-chevron-right-20-solid" size="lg" color="white" :ui="{ rounded: 'rounded-full dark:border-neutral-800' }" class="rtl:[&_span:last-child]:rotate-180 dark:bg-neutral-900 border-[0.5px]" @click="onClick" />
                    </template>
                  </UPagination>
                </div>
              </div>
...
const totalPages = computed(() => {
  ...
  const endItem = computed(() => Math.min(page.value * rowsPerPage.value, totalItems.value));
