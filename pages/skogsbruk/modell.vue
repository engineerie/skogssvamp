...
export default {
  ...
  setup() {
    ...
    function onViewerOpened(source) {
      ...
      // After a viewer opens (meaning new image loaded), re-apply the global viewport
    ...
    function getActiveViewers() {
      if (!isCompare.value && !isFrameworkCompareMode.value) {
        ...
        return [
          beforeViewerRef.value?.viewer,
          afterViewerRef.value?.viewer,
        ].filter(Boolean);
    ...
    function zoomAllIn() {
      ...
      // Update globalViewport with the new zoom
      if (viewers.length > 0) {
    ...
    function resetAll() {
      ...
      // After reset, update globalViewport to default
    ...
    return {
      ...
      selectedFrameworkIndex2,
      startskogModel,
      time,
      sliderSteps,
      currentTimeLabel,
      handleDisabledToggleFrameworkCompareClick,
      showPingEffectCompareButton,
      comparisonImagePath1,
      currentImagePath,
      currentImagePath2,
      timeLabelForDataFiltering,
      timeLabelForDataFiltering2,
      onViewportChanged,
      onViewerOpened,
      frameworks,
      startskog,
